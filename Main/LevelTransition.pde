    ...
    for (int i=0; i<shieldPiece1Initializer[1].length; i++) {
      for (int j=0; j<shieldPiece1Initializer.length; j++) {
        if (shieldPiece1Initializer[j][i]==1) {
          shieldPiece1Array[i][j] = new ShieldPiece(SCREENX/5-.5*shieldPiece1Initializer[1].length*SHIELD_PIECE_WIDTH+i*SHIELD_PIECE_WIDTH, SCREENY-4*MARGIN+j*SHIELD_PIECE_HEIGHT);
          shieldPiece1Array[i][j].draw();
        }
        if (shieldPiece2ArrayInitialiser[j][i]==1) {
          shieldPiece2Array[i][j] = new ShieldPiece(2*SCREENX/5-.5*shieldPiece1Initializer[1].length*SHIELD_PIECE_WIDTH+i*SHIELD_PIECE_WIDTH, SCREENY-4*MARGIN+j*SHIELD_PIECE_HEIGHT);
          shieldPiece2Array[i][j].draw();
        }
        if (shieldPiece3ArrayInitialiser[j][i]==1) {
          shieldPiece3Array[i][j] = new ShieldPiece(3*SCREENX/5-.5*shieldPiece1Initializer[1].length*SHIELD_PIECE_WIDTH+i*SHIELD_PIECE_WIDTH, SCREENY-4*MARGIN+j*SHIELD_PIECE_HEIGHT);
          shieldPiece3Array[i][j].draw();
        }
        if (shieldPiece4ArrayInitialiser[j][i]==1) {
          shieldPiece4Array[i][j] = new ShieldPiece(4*SCREENX/5-.5*shieldPiece1Initializer[1].length*SHIELD_PIECE_WIDTH+i*SHIELD_PIECE_WIDTH, SCREENY-4*MARGIN+j*SHIELD_PIECE_HEIGHT);
  ...
  void gameOver(Invader1[] invaders1Array, Invader2[] invaders2Array, Invader3[] invaders3Array, Lives lives){
    ...
    for (int i=0; i<shieldPiece1Initializer[1].length; i++){
      for (int j=0; j<shieldPiece1Initializer.length; j++){
        ...
        if (shieldPiece2ArrayInitialiser[j][i]==1){
          shieldPiece2Array[i][j] = new ShieldPiece(2*SCREENX/5-.5*shieldPiece1Initializer[1].length*SHIELD_PIECE_WIDTH+SCREENX/4+j*SHIELD_PIECE_WIDTH, SCREENY-4*MARGIN+j*SHIELD_PIECE_HEIGHT);
          shieldPiece2Array[i][j].draw();
        }
        if (shieldPiece3ArrayInitialiser[j][i]==1){
          shieldPiece3Array[i][j] = new ShieldPiece(3*SCREENX/5-.5*shieldPiece1Initializer[1].length*SHIELD_PIECE_WIDTH+SCREENX/2+i*SHIELD_PIECE_WIDTH, SCREENY-4*MARGIN+j*SHIELD_PIECE_HEIGHT);
          shieldPiece3Array[i][j].draw();
        }
        if (shieldPiece4ArrayInitialiser[j][i]==1){
          shieldPiece4Array[i][j] = new ShieldPiece(4*SCREENX/5+SCREENX/2-.5*shieldPiece1Initializer[1].length*SHIELD_PIECE_WIDTH+SCREENX/2+i*SHIELD_PIECE_WIDTH, SCREENY-4*MARGIN+j*SHIELD_PIECE_HEIGHT);
          shieldPiece4Array[i][j].draw();
        }
      }
    }
